<template>
  <div class="container-water-fall">
    <top-bar>
        <span slot="topText" class="top-text">匹配结果</span>
    </top-bar>
    ...
    <waterfall :col="col" :data="data" @loadmore="loadmore">
      <template>
        <div class="cell-item" v-for="(item,index) in data" :key="index">
          <img v-if="item.matchedFaceUrl" :lazy-src="item.matchedFaceUrl" alt="加载错误" @click="viewNormalSzie(item.msUrl)">
          <div class="item-body">
            <div class="item-desc">
...
<script>
  import {uf2ufMatch} from '@/api/picture/picture.js'
  import {Notify,Tag,ImagePreview,Popup,Slider,Form,Field,Dialog,PullRefresh,Icon,Loading } from 'vant';
  ...
  export default {
    ...
    created() {
      this.userFaceId = this.$route.params.userFaceId;
      this.getPictureList();
    },
    mounted(){
      //监听浏览器返回事件
