<template>
  <div>
    <top-bar>
        <span slot="topText" class="top-text">我的相册</span>
    </top-bar>
    <van-search
      v-model="name"
      show-action
      ...
      <template #action>
        <slot name="share" >
          ...
          -->
          <van-icon name="add" size="25px" color="#099" @click="handleUpload" slot="leftIcon"/>
        </slot>
      </template>
    </van-search>
      ...
      <waterfall :col="col" :data="data" @loadmore="loadmore"  >
        <template>
          <div class="cell-item" v-for="(item,index) in data" :key="index">
            <img :lazy-src="item.url" alt="加载错误" @click="viewOrigin(item.originalUrl)">
            <div class="item-body">
              <div class="item-desc" @click="handleEdit(item.id)">
              {{item.title}}
              </div>
              <div class="item-footer">
                <div class="avatar">
                  <img src="@/assets/image/face.png" @click="showDetail(item.id)" />
                </div>
                <div class="name" @click="showDetail(item.id)" >
                  {{item.shotTime.substring(0,10)}}
                </div>
                <div class="like">
                  <i></i>
                  <div class="pic-delete" @click="handleDelete(item.id)">删除</div>
                </div>
              </div>
            </div>
          </div>
...
*/
  import loading from "@/components/waterfall/loading";
  import {fetchList} from '@/api/picture/picture.js'
  import {Notify,ImagePreview,Dialog,PullRefresh,Search,Icon,Loading} from 'vant';
  ...
  export default {
    ...
    methods: {
      ...
      /**查看人脸*/
      showDetail(id){
        this.$router.push({name:'PictureFace',params: {id:id}});
      },
      /**编辑*/
      handleEdit(id){
