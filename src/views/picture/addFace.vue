...
<template>
  <div>
    <top-bar>
        <span slot="topText" class="top-text">添加人脸</span>
    </top-bar>
...
<script>
  ...
  import { addFace } from '@/api/picture/pictureFace.js'
  ...
  methods: {
    ...
    getPicture(id) {
      ...
      getDetail(id).then(response => {
        this.option.img=response.originalUrl;
      }).catch(() => {
      });
    },
    /**上传图片*/
    uploadFace() {
      this.$refs.cropper.getCropBlob(data => {
        ...
        formData.append("pictureId", this.pictureId);
        addFace(formData).then(response => {
          this.$router.push({name:'PictureFace',params: {id:this.pictureId}})
        });
      });
    },
